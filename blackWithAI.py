...
#P.S there is a lot of unimportant code here that is neccessary for PvP but since the AI doesn't use
#mousePressed for example, all of the code checking for white pieces is unimportant
...
def appStarted(app):
    ...
    bestEval, bestMove = minimax(app, app.board,  3, float('-inf'), float('inf'))
    row, col, newRow, newCol = bestMove
    movePiece(app, row, col, newRow, newCol)
    app.pastMoves.append((app.board[newRow][newCol], row, col, newRow, newCol))
    ...
    if app.gameOver:
        if app.checkmate:
            ...
            canvas.create_text(app.width/4, 25, text = "Stalemate! Press 'r' to restart", fill = 'white', font = 'ITF 16 bold')
             ...
             canvas.create_text(app.width/4, 25, text = "The computer resigns! Press 'r' to restart", fill = 'white', font = 'ITF 16 bold')
...
def drawNotation(app, canvas):
    ...
    for i in range(app.rows):
        canvas.create_text(2*app.margin/3, app.margin + cellWidth/2 + (i*cellHeight), text = 1 + i,
    ...
    for i in range(app.cols):
        canvas.create_text(app.margin + cellWidth/2 + (i*cellHeight), app.margin + gridHeight + 5,
                           text = chr(ord('h')-i), fill = 'white', font = 'ITF 16 bold', anchor = 'n')


def drawComputer(app, canvas):
    if app.whitesMove and not app.gameOver:
        canvas.create_text(app.width/4, 25, text = "The computer is thinking!", fill = 'white', font = 'ITF 16 bold')
...
def mousePressed(app, event):
    ...
    #(this short-circuits if you aren't supposed to move the piece)
        ...
        if app.board[row][col] == 'P' and row == 7 and not isKingCurrentlyInCheck(app, kingRow, kingCol):
            ...
            if isInCheckmate(app):
                app.pastMoves.append((app.board[row][col], app.startRow, app.startCol, row, col))
                app.gameOver = True
        #If a black pawn has reached the end of the board, make it a piece the user chooses 
        elif app.board[row][col] == 'p' and row == 0 and not isKingCurrentlyInCheck(app, kingRow, kingCol):
            app.board[row][col] = choosePiece(app, '')
            canPlayerCastle(app, row, col)
            isPlayerCastling(app, row, col)
            app.whitesMove = not app.whitesMove
        ...
        elif app.board[row][col] == 'P' and col == app.startCol + 1 and not isKingCurrentlyInCheck(app, kingRow, kingCol):
        ...
        #If the king is trying to castle and it will be in check if it castle, reset the move(next 4 if-statements)
        elif app.startRow == 7 and app.startCol == 3 and row == 7 and col == 5 and (isKingCurrentlyInCheck(app, kingRow, kingCol) or isKingCurrentlyInCheck(app, kingRow, kingCol-1 ) or isKingCurrentlyInCheck(app, kingRow, kingCol-2) or 'P' in app.board[6][2:6]):
            app.board[app.startRow][app.startCol] = app.board[row][col]
            app.board[row][col] = capturedPiece
            (app.startRow, app.startCol) = (None, None)
        elif app.startRow == 7 and app.startCol == 3 and row == 7 and col == 1 and (isKingCurrentlyInCheck(app, kingRow, kingCol) or isKingCurrentlyInCheck(app, kingRow, kingCol+1) or isKingCurrentlyInCheck(app, kingRow, kingCol+2) or 'P' in app.board[6][0:4]):
            app.board[app.startRow][app.startCol] = app.board[row][col]
            app.board[row][col] = capturedPiece
            (app.startRow, app.startCol) = (None, None)
        elif app.startRow == 0 and app.startCol == 3 and row == 0 and col == 5 and (isKingCurrentlyInCheck(app, kingRow, kingCol) or isKingCurrentlyInCheck(app, kingRow, kingCol-1) or isKingCurrentlyInCheck(app, kingRow, kingCol-2) or 'p' in app.board[1][2:6]):
            app.board[app.startRow][app.startCol] = app.board[row][col]
            app.board[row][col] = capturedPiece
            (app.startRow, app.startCol) = (None, None)
        elif app.startRow == 0 and app.startCol == 3 and row == 0 and col == 1 and (isKingCurrentlyInCheck(app, kingRow, kingCol) or isKingCurrentlyInCheck(app, kingRow, kingCol+1) or isKingCurrentlyInCheck(app, kingRow, kingCol+2) or 'p' in app.board[1][0:4]):
            app.board[app.startRow][app.startCol] = app.board[row][col]
            app.board[row][col] = capturedPiece
            (app.startRow, app.startCol) = (None, None)
...
#Checks if it's AI's turn constantly and plays if it is
def timerFired(app):
    if app.whitesMove:
        bestEval, bestMove = minimax(app, app.board,  3, float('-inf'), float('inf'))
        if bestMove == None:
        ...
        row, col, newRow, newCol = bestMove
        movePiece(app, row, col, newRow, newCol)
        isAICastling(app, row, col, newRow, newCol)
        if app.board[newRow][newCol] == 'p' and newRow == 0:
            app.board[newRow][newCol] = 'q'
        elif app.board[newRow][newCol] == 'P' and newRow == 7:
            app.board[newRow][newCol] = 'Q'
        app.pastMoves.append((app.board[newRow][newCol], row, col, newRow, newCol))
        if isInCheckmate(app):
            app.gameOver = True
...
def getMoveNotation(app, pieceSelected, row, col):
    rowNotation = str(1 + row)
    colNotation = chr(ord('h') - col)
...
def isAICastling(app, row, col, newRow, newCol):
    if not app.whitesMove:
        if (row == 7 and col == 3) and (newRow == 7 and newCol == 5) and app.whiteCanCastleRightSide:
            movePiece(app, 7, 7, 7, 4)
        elif (row == 7 and col == 3) and (newRow == 7 and newCol == 1) and app.whiteCanCastleLeftSide:
            movePiece(app, 7, 0, 7, 2)
    else:
        if (row == 0 and col == 3) and (newRow == 0 and newCol == 5) and app.blackCanCastleRightSide:
            movePiece(app, 0, 7, 0, 4)
        elif (row == 0 and col == 3) and (newRow == 0 and newCol == 1) and app.blackCanCastleLeftSide:
            movePiece(app, 0, 0, 0, 2)
...
def canPlayerCastle(app, newRow, newCol):
    if not app.whitesMove:
        if (app.startRow == 7 and app.startCol == 3) and (newRow != 7 and newCol != 5):
            app.whiteCanCastleRightSide = False
        elif (app.startRow == 7 and app.startCol == 3) and(newRow != 7 and newCol != 1):
        ...
        if (app.startRow == 0 and app.startCol == 3) and (newRow != 0 and newCol != 5):
            app.blackCanCastleRightSide = False
        elif (app.startRow == 0 and app.startCol == 3) and (newRow != 0 and newCol != 1):
...
def isPlayerCastling(app, newRow, newCol):
    if not app.whitesMove:
        if (app.startRow == 7 and app.startCol == 3) and (newRow == 7 and newCol == 5) and app.whiteCanCastleRightSide:
            movePiece(app, 7, 7, 7, 4)
        elif (app.startRow == 7 and app.startCol == 3) and (newRow == 7 and newCol == 1) and app.whiteCanCastleLeftSide:
            movePiece(app, 7, 0, 7, 2)
    else:
        if (app.startRow == 0 and app.startCol == 3) and (newRow == 0 and newCol == 5) and app.blackCanCastleRightSide:
            movePiece(app, 0, 7, 0, 4)
        elif (app.startRow == 0 and app.startCol == 3) and (newRow == 0 and newCol == 1) and app.blackCanCastleLeftSide:
            movePiece(app, 0, 0, 0, 2)
...
def getLegalKingMoves(app, startRow, startCol):
    ...
    if not app.whitesMove:
        if startRow == 7 and startCol == 3 and app.whiteCanCastleRightSide:
            if app.board[7][1] == '_' and app.board[7][2] == '_':
                    legalMoves.add((7, 1))
        if startRow == 7 and startCol == 3 and app.whiteCanCastleLeftSide:
            if app.board[7][4] == '_' and app.board[7][5] == '_' and app.board[7][6] == '_':
                    legalMoves.add((7, 5))
    else:
        if startRow == 0 and startCol == 3 and app.blackCanCastleRightSide:
            if app.board[0][1] == '_' and app.board[0][2] == '_':
                legalMoves.add((0, 1))
        if startRow == 0 and startCol == 4 and app.blackCanCastleLeftSide:
            if app.board[0][4] == '_' and app.board[0][5] == '_' and app.board[0][6] == '_':
                legalMoves.add((0, 5))
...
#However, we still run into an infinite loop if we don't make sure we only check this once
#We also have to make sure we only check the legal moves of the opponent's pieces
#So all we have to do is say shouldCheckOpponentsLegalMoves = False
...
#I looked at https://en.wikipedia.org/wiki/Minimax to understand the concepts. 
#For alpha-beta pruning I looked at https://en.wikipedia.org/wiki/Alpha%E2%80%93beta_pruning and https://www.youtube.com/watch?v=xBXHtz4Gbdo
#I looked at pseudocode in these wikipedia articles
def minimax(app, board, depth, alpha, beta):
    if depth == 0:
        return (evaluateBoard(app), None)

    if app.whitesMove:
        biggest = float('-inf')
        oldBiggest = float('-inf')
        bestMove = None
        for row in range(app.rows):
            for col in range(app.cols):
                if app.board[row][col] not in app.whitesPieces:
                    continue
                legalMoves = getLegalMoves(app, row, col)
                for legalMove in legalMoves:
                    if checkForTrueLegality(app, row, col, legalMove):
                        newRow, newCol = legalMove
                        board = copy.deepcopy(app.board)
                        movePiece(app, row, col, newRow, newCol)
                        app.whitesMove = not app.whitesMove
                        biggest = max(biggest, minimax(app, app.board, depth-1, alpha, beta)[0])
                        app.whitesMove = not app.whitesMove
                        app.board = board
                        if biggest > oldBiggest:
                            oldBiggest = biggest
                            bestMove = (row, col, newRow, newCol)
                        alpha = max(biggest, alpha)
                        if alpha >= beta:
                            break
        return (biggest, bestMove)

    else:
        smallest = float('inf')
        oldSmallest = float('inf')
        bestMove = None
        for row in range(app.rows):
            for col in range(app.cols):
                if app.board[row][col] not in app.blacksPieces:
                    continue
                legalMoves = getLegalMoves(app, row, col)
                for legalMove in legalMoves:
                    if checkForTrueLegality(app, row, col, legalMove):
                        ...
                        movePiece(app, row, col, newRow, newCol)
                        app.whitesMove = not app.whitesMove
                        smallest = min(smallest, minimax(app, app.board, depth-1, alpha, beta)[0])
                        app.whitesMove = not app.whitesMove
                        app.board = board
                        ...
                        if alpha >= beta:
                            break
        return (smallest, bestMove)

#Gets legal moves before check
def getLegalMoves(app, startRow, startCol):
    pieceSelected = app.board[startRow][startCol]
    setOfAllLegalMoves = set()
    if pieceSelected == 'p' or pieceSelected == 'P':
        setOfAllLegalMoves = getLegalPawnMoves(app, startRow, startCol)
        ...
        setOfAllLegalMoves = getLegalKingMoves(app, startRow, startCol)
    return setOfAllLegalMoves


#checks if its a legal move including checks
def checkForTrueLegality(app, row, col, move):
    newRow, newCol = move
    capturedPiece = app.board[newRow][newCol]
    movePiece(app, row, col, newRow, newCol)
    ...
    if row == 7 and col == 4 and newRow == 7 and newCol == 6 and (isKingCurrentlyInCheck(app, kingRow, kingCol) or isKingCurrentlyInCheck(app, kingRow, kingCol - 1) or isKingCurrentlyInCheck(app, kingRow, kingCol - 2) or 'p' in app.board[6][3:6]):
        movePiece(app, newRow, newCol, row, col)
        app.board[newRow][newCol] = capturedPiece 
        return False
    elif row == 7 and col == 4 and newRow == 7 and newCol == 2 and (isKingCurrentlyInCheck(app, kingRow, kingCol) or isKingCurrentlyInCheck(app, kingRow, kingCol + 1) or isKingCurrentlyInCheck(app, kingRow, kingCol + 2) or 'p' in app.board[6][1:5]):
        movePiece(app, newRow, newCol, row, col)
        app.board[newRow][newCol] = capturedPiece 
        return False
    elif row == 0 and col == 4 and newRow == 0 and newCol == 2 and (isKingCurrentlyInCheck(app, kingRow, kingCol) or isKingCurrentlyInCheck(app, kingRow, kingCol - 1) or isKingCurrentlyInCheck(app, kingRow, kingCol - 2) or 'P' in app.board[1][1:5]):
        movePiece(app, newRow, newCol, row, col)
        app.board[newRow][newCol] = capturedPiece 
        return False
    elif row == 0 and col == 4 and newRow == 0 and newCol == 2 and (isKingCurrentlyInCheck(app, kingRow, kingCol) or isKingCurrentlyInCheck(app, kingRow, kingCol + 1) or isKingCurrentlyInCheck(app, kingRow, kingCol + 2) or 'P' in app.board[1][3:6]):
        movePiece(app, newRow, newCol, row, col)
        app.board[newRow][newCol] = capturedPiece 
        return False
    elif isKingCurrentlyInCheck(app, kingRow, kingCol):
...
#calculates the current nodes eval and returns that number
#I chose this evaluation based on what I think is important in chess
def evaluateBoard(app):
    total = 0
    for row in range(app.rows):
        for col in range(app.cols):
            weight = 1
            if app.board[row][col] != '_':
                piece = app.board[row][col]
                if (row >= 2 and row <= 4) and (col >= 2 and col <= 4) and (piece == 'p' or piece == 'P'):
                    weight = 1.5
                if (row >= 2 and row <= 4) and (col >= 2 and col <= 4) and (piece == 'n' or piece == 'N'):
                    weight = 1.166
                if (row >= 2 and row <= 4) and (col >= 2 and col <= 4) and (piece == 'b' or piece == 'B'):
                    weight = 1.166
                if (row >= 2 and row <= 4) and (col >= 2 and col <= 4) and (piece == 'r' or piece == 'R'):
                    weight = 1.1
                if (row >= 2 and row <= 4) and (col >= 2 and col <= 4) and (piece == 'q' or piece == 'Q'):
                    weight = 1.055
                if len(app.pastMoves) < 8 and row != 7 and piece == 'q':
                    weight = .625
                if len(app.pastMoves) < 8 and row != 0 and piece == 'Q':
                    weight = .625
                total += app.pieceValues[piece]*weight
    return total
